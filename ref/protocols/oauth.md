...
A scope is the mechanism used in OAuth to describe and limit the access granted to an application to the protected resources.

Access tokens always include the authorized scopes and that is how the resource server is able to validate access tokens and decide on whether to accept requests from clients or not.

When applicable, these scopes are presented to the end-user in the consent screen, where he grants authorization or refuses it.

Scopes may be required, meaning that the client application can only operate correctly if those are grant, or optional, in the sense that the client application may still be able to perform part of its function without them.
...
There are 2 kinds of clients, depending on their ability to maintain confidentiality of their credentials):
...
An authorization grant (or authorization flow or grant type) defines a credential that represents the authorization given by the resource owner that may be used by the client to obtain access tokens.
...
Instead of requesting authorization directly from the resource owner, the client directs the resource owner to the authorization server (via the browser), which in turn redirects the resource owner back to the client with an authorization code.

Before redirecting the resource owner back to the client, the authorization server authenticates the resource owner and obtains authorization.

> Since the resource owner only authenticates with the authorization server, its credentials are never shared with the client.

This flow offers important security benefits:

- The ability to authenticate the client.
- The access token is transmitted directly from the authority server to the client without passing it through the resource owner's browser (user agent), which could potentially exposit it to others (including the resource owner).
...
- When the client initiates the authorization request, instead of immediately launching a browser, it first creates a "code verifier" (a random string with between 43 and 128 characters). It then uses it to create a "code challenge" (an hash calculated on the code verifier) and includes it in the authorization request.
- The authorization server will recognize this code challenge and associate it (and store it) with the authorization code that it issues. It then returns the authorization code as normal (without the code challenge).
...
- The authorization server will then be able to validate the code verifier agains the code challenge received previously (along with all the other parameters validated in the Authorization Code flow) and only issue the access token if there is a match.
...
Instead of issuing an authorization code, the authorization server issues the access token directly as the result of authorization, and it does not authenticate the client (although its identity may be verifying indirectly via the redirect URI used to deliver the access token).

> This authorization grant has been [recommended for deprecation](https://tools.ietf.org/html/draft-ietf-oauth-security-topics-09). It should NOT be used (use Authorization Code instead).

### Client Credentials

Client credentials are used when the client is acting on its own behalf, in machine-to-machine scenarios with no end-users involved.

This is the case when the client is also the resource owner or when the client is requested access to resources based on some authorization previously arranged (granted) with the authorization server.
...
> This grant flow is suited for confidential clients, not to public clients.

### Resource Owner Password

This grant type uses the resource owner password credentials (the user name and password) to obtain access token.
